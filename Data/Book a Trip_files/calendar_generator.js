        ...
        $A(
						[
						{
						    ...
						    runBefore: function (dc) {
						        ...
						        //added option to set current date - selectedDate
						        var accCalData = $("#" + dc.id).data("accdc.calendar");
						        if (accCalData != undefined && accCalData.flag == 0) {
						            dc.date = accCalData.selectedDate;
						            dc.setCurrent(dc);
						            accCalData.flag = 1;
						            $("#" + dc.id).data("accdc.calendar", accCalData);
						        }

						        //added option to disable specified days - disabledDates
						        if ($("#" + dc.id).data("accdc.calendar") != undefined) {
						            var disabledDates = $("#" + dc.id).data("accdc.calendar").disabledDates;
						            for (var month in disabledDates) {
						                for (var year in disabledDates[month].disabled) {
						                    dc.range[month].disabled[year] = disabledDates[month].disabled[year];
						                }
						            }
						        }
						    ...
						    runAfter: function (dc) {
						        ...
						        dateNow = new Date(yearNow, monthNow, dayNow);
						        maxDate = new Date(yearNow, monthNow, dayNow + 548);
						        var nMonth = function () {
						            ...
						            // condition for disabling next month VS max month
						            if (dc.date <= maxDate) {
						                dc.setCurrent(dc);
						                dc.reopen = true;
						                dc.open();
						                ...
						                pMonth = function () {
						                    var month = dc.range.current.month < 1 ? 11 : dc.range.current.month - 1,
                                                ...
                                                day = dc.range.current.mDay > dc.range[month].max ? dc.range[month].max : dc.range.current.mDay;
						                    dc.date = new Date(year, month, day);
						                    // condition for disabling prev month VS present month
						                    if (dc.date >= dateNow) {
						                        dc.setCurrent(dc);
						                        dc.reopen = true;
						                        dc.open();
						                    ...
						                    // conditions for disabling prev year VS present year & nxt year vs maxdate of 548 days
						                    enableYrBtn = true;
						                    if ((year < yearNow && forward == undefined) ||
                                                (year > maxDate.getFullYear() && forward != undefined)) {
						                        enableYrBtn = false;
						                    }
